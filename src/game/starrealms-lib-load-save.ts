...
import { addLogEntry, rebuildTurnStatisticsCache } from '@/game/starrealms-lib-log';
...
export function isValidGame(game: unknown): game is IGame {
  ...
  const isValidSupply = (supply: unknown): supply is IGameSupply => {
    ...
    return (
      typeof s.explorers === 'number' &&
      typeof s.scouts === 'number' &&
      typeof s.vipers === 'number'
    );
  };
